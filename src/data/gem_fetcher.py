"""Open-Meteo GEM/GEPS Ensemble (21-member) data fetcher.

Fetches temperature_2m from the Canadian GEM ensemble (gem).
Response contains 21 members (member00-member20).

GEM runs every 12h at 00Z and 12Z. Data available ~6h after run start.
Resolution: 25km, 3-hourly data.
"""
...
from src.data.models import GemForecast
...
class GemResult:
    ...
    model_run_time: str
    target_date: str
    member_daily_maxes: list[float]  # 21 daily max temps in F, one per member
    valid_times: list[str]


class GemFetcher:
    """Fetches GEM/GEPS ensemble forecasts from Open-Meteo."""
    ...
    def fetch_ensemble(
        ...
        """Fetch GEM ensemble for a city and extract daily max per member."""
        params = {
            "latitude": city.lat,
            "longitude": city.lon,
        ...
        try:
            ...
            logger.error(f"GEM fetch failed for {city.name}: {e}")
            return []

        return self._parse_response(data, city)

    def _parse_response(self, data: dict, city: CityConfig) -> list[GemResult]:
        """Parse Open-Meteo GEM response into daily max per member."""
        hourly = data.get("hourly", {})
        time_strings = hourly.get("time", [])
        if not time_strings:
            logger.warning(f"No time data in GEM response for {city.name}")
            return []

        times = [parse_iso_datetime(t) for t in time_strings]

        # Extract member data (member00 through member20)
        member_data: dict[int, list[float]] = {}
        for m in range(GEM_MEMBERS):
            if m == 0:
                key = "temperature_2m"
        ...
        for target_date in sorted(target_dates):
            ...
            # GEM has only 21 members; require at least 10 valid
            valid_count = sum(1 for t in member_maxes if t == t)
            if valid_count < 10:
                logger.warning(
                    f"Only {valid_count} valid GEM members for "
    ...
    def store_ensemble(self, results: list[GemResult], city: CityConfig) -> int:
        """Store GEM forecast data in the database."""
        session = get_session()
        inserted = 0
    ...
    def fetch_and_store(self, city: CityConfig, forecast_days: int = 3) -> list[GemResult]:
        """Fetch GEM data and store in database."""
        results = self.fetch_ensemble(city, forecast_days)
        if results:
            count = self.store_ensemble(results, city)
            logger.info(
                f"Stored {count} new GEM records for {city.name} ({len(results)} dates)"
            )
        return results
